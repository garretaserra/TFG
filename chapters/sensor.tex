...
Un cop realitzat l'experiment s'han validat, tal com s'havia comentat anteriorment, els avantatges que aporta la nova capa física Coded que no existia abans de la versió 5.0 de BLE.
En aquest rang amb visibilitat directa es pot considerar que BLE podrà assolir cobertura suficient dins les cases a través de parets.
...
Un cop s'estableix una connexió amb el dispositiu es realitza una altra captura d'un segon que es pot observar a la figura \ref{energy_trace2}.
...
Aquesta aplicació llegeix contínuament els valors dels quatre atributs que té la PCB relacionats amb els sensors.
El codi de l'app es pot trobar a \cite{android_repo}.
