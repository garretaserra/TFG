...
EL Bluetooth Low Energy permet la connectivitat punt a punt i això no és suficient per poder competir amb les altres tecnologies.
Si tenim un mòbil que es connecta amb múltiples dispositius BLE, aquests no es poden comunicar entre si i això limita l'ús de BLE en desplegaments grans.
En canvi, amb altres MANETs que s'han vist a l'apartat \ref{MANETS}, com Zigbee, 6LoWPAN, Z-Wave, entre d'altres, estableixen mecanismes que permeten connectar tots els nodes entre si en una xarxa.

EL Bluetooth necessitava establir alguna manera per poder implementar xarxes complexes entre nodes i ho va fer amb el \textit{Mesh Profile}.
...
En aquest estàndard és defineix el perfil, els models i les propietats necessàries per permetre una xarxa mallada.
EL gran avatatge que suposa es que a la pràctica pot incrementar l'abast de dispositius ja que es poden utilitzar altres nodes siguin sensors o dispositius específics, que realitzin les funcions com a repetidors.
...
Amb la introducció de BLE sorgeix la possibilitat d'enviar un mateix missatge amb potencialment múltiples disposistius com a destí, per exemple, utilitzant missatges d'anunci.
...
Gràcies al nou \textit{Mesh Profile} un node pot ser mestre en una connexió i esclau en una altre.
...
[Explicar escenes]
Cal destacar que tot i que probablement la majoria de xarxes que utilitzin el \textit{Mesh Profile} no seran molt grans, l'estàndard s'ha dimensionat per poder tenir xarxes extremadament grans, pensades per parquings de cotxes, control d'ilumincació en edificis d'oficines o sensors en caps de futbol entre d'altres.
Els límits del dimensionament són de 32.767 nodes per xarxa, fins a 4.096 subxarxes i 65.535 escenes que vénen marcats per la longitud dels camps que identifiquen l'estructura que són de 15, 12 i 16 bits respectivament.
...
És per això que el \textit{Mesh profile} inclou la possibilitat de definir un node com a "amic".
...
En la pila del \textit{Mesh Profile}  conté tota la pila de BLE a la capa inferior ja que el \textit{mesh profile} utilitza BLE per enviar paquets entre els nodes.
A continuació es comentarà breument les diferents capes d'aquesta pila.
...
La capa \textit{Bearer} controla la transmissió i recepció de paquets, n'hi ha de dos tipus l'\textit{Advertising Bearer} i el GATT \textit{Bearer}.
L'\textit{Advertising Bearer} utiliza el BLE per enviar i rebre missatges corresponents al \textit{Mesh Profile}.
En canvi, el GATT \textit{Bearer} és el que s'utilitza en sipositius que no implementen el \textit{Mesh Profile} i permet comunicar-se indirectament amb altres dispositius de la xarxa utilitzant el protocol d'intermediari comentat anteriorment.
...
La capa de transport inferior realitza la segmentació i reesemblatge dels paquets que no caben en una PDU de transport.
La capa de transport superior es responsable de la seguretat proveïnt de encriptació i autenticació.
Igualment també s'encarrega de la funcionalitat d'amistat entre dispositius comentada anteriorment.
La capa d'accès es defineix el format de les dades de la capa de transport, quina encriptació s'ha de realitzar entre d'altres.
Les dues capes superiors tracten la implementació dels models i els missatges, estats que defineixen.
...
Això es deu a que enlloc de substituïr o extendre BLE amb el \textit{Mesh Profile} el que s'ha fet es mantenir BLE tal i com està i afegir \textit{Mesh Profile} per sobre.
...
En la majoria de tecnologies que s'han vist fins ara, incloent BLE la seguretat és opcional per permetre implementacions simples del protocol que no ho requereixen.
Aquest no és el cas amb el \textit{Mesh Profile} que obliga a utilitzar seguretat segons l'estàndard.

Tots els paquets corresponents al \textit{Mesh Profile} estan encriptat i autenticats.
Existeixen les claus de xarxa que són compartides entre tots els nodes de la xarxa i es requereixen per realitzar funcions de xarxa com reenviar paquets.
Amb aquestes claus s'obfusquen les capçaleres del paquet per dona privacitat en l'origen i destí d'aquest als nodes que no són de la xarxa.
Si un node forma part d'una subxarxa, aquest també tindrà una altre clau única i compartida entre els nodes de la subxarxa.
Per tant, es poden aïllar certes parts de la xarxa, per exemple, una estancia d'un d'hotel.

Cada xarxa pot tenir múltiples aplicacions que tindran la seva pròpia clau d'aplicació.
Aquesta servira per aïllar diferents elements d'una xarxa que estan relacionats com poden ser els elements d'iluminació (llums i interruptors) dels elements de calefacció (sensor de temperatura i radiador).

Finalment, tots els nodes contenen una clau de dispositiu que els permet formar part d'una xarxa o una aplicació.
Quant un dispositiu es vol eliminar de la xarxa o aplicació, per exemple, per vendre'l, es procedeix de forma segura afegint aquella clau a una llista negre.
Els dispositius que estan a la llista negre no se'ls renoven les claus d'aplicació o xarxa i per tant no poden seguir participant en les communicacions.
Tal com és habitual d'utilitzen identificadors seqüencials en els paquets per evitar atacs de repetició en que l'atacant retransmet un paquet per intentar repetir una acció anterior.
