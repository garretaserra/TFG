...
Altres tecnologies similars, que s'han vist a l'apartat \ref{MANETS}, com Zigbee, 6LoWPAN, Z-Wave, entre d'altres, estableixen mecanismes per connectar tots els nodes en una xarxa.
...
En la pila del \textit{Mesh Profile}  conté tota la pila de BLE a la capa inferior ja que BLE proporciona les comunicacions al perfil \textit{mesh}.
...
\begin{figure}[!h]
	\begin{center}
		...
		\caption{Pila Xarxa Mallada \cite{Mesh Profile Overview}}
		\label{mesh_netowrk_stack}
	\end{center}
\end{figure}

Tal i com es pot veure en la Figura \ref{mesh_netowrk_stack} el controlador, format per les dues capes inferiors, és manté igual.
