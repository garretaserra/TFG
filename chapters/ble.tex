...
Per utilitzar-los es necessari respectar la normativa nacional que normalment assimila la normativa comuna segons la ITU\footnote{La Internation Telecomunications Union depen de les nacions unides i s'encarrega d'assignar l'espectre radioelèctric per a comunicacions.}.
Aquestes bandes es designen ISM, acrònim d'\textit{Industrial}, \textit{Scientific} i \textit{Medical}, que són els usos pels que s'havia pensat que servirien originalment, avui en dia aquests usos són més diversos.
...
El desenvolupament de la tecnologia per a connexions de curt abast que acabarien esdevenint el que avui es coneix com a Bluetooth Clàssic va començar l'any 1989 per part de la empresa Ericsson.
...
Els membres del SIG després de negociar entre sí, van acordar incloure Wibree al estàndard de Bluetooth en l'especificació 4.0 amb el nom de \textit{Bluetooth ultra low power technology} i publicitat com a Bluetooth Smart. El primer mòbil a incloure'l va ser l'iPhone 4S que va sortir al mercat al 2011.
...
A més a més té el desavantatge que, tot i voler transmetre poques dades, l'arquitectura fa que es consumeixi bastanta energia per establir i mantenir la connexió, encara que sigui breu.
...
Més endavant també es va fer popular l'us del Bluetooth per a escoltar música a través d'auriculars inal·làmbrics.
En aquests casos el cost d'establiment de la connexió és insignificant en comparació amb el cost mentre s'estan tranferint dades, que acostuma a ser un període llarg de temps.
...
\footnotetext{L'IEEE 802.15.4 és un estandàrd orientat a xarxes inal·làmbriques amb una baixa tassa de dades. Aquest només defineix les capes físiques i d'accès al medi.}
...
\subsubsection{LoRa}
El portocol Long Range desenvolupat per Semtech està orientat a cobrir les distàncies més grans d'entre les MANETs.
...
Tot i que els protocols tenen molt en comú, com per exemple el se ús en domòtica, cada un es diferència de la resta en els detalls.
A la taula \ref{taula_comparacio} es pot veure una comparació de les capacitats de cada un dels protocols.
Cal mencionar que les freqüències de les bandes mencionades poden variar lleugermanet segons la normativa de cada país.
En la comparació es pot veure com BLE és una de les que té millors capcitats.
Això es deu a que tot i que no es possible aconseguir totes les característiques alhora la seva flexibilitat permet configurar el protocol per a moltes configuracions diferents.
...
\begin{table}[h]
	...
	\resizebox{\textwidth}{!}{
		\begin{tabular}{|r|c|c|c|c|c|}
			\hline
&Abast (m)&	Velocitat Física (Kbps)	&	Tasa de dades (Kbps)   & Consum (mA) & Banda    \\ \hline
...
\label{Versions_BLE}
Tal i com s'ha vist anteriorment, BLE és classifica segons les seves capacitats en la seva versió 4 i en la 5.
...
Per evitar les possibles interferències que causin altres tecnologies, es defineix la SAM (\textit{Slot Availability Mask}) que permet identificar aquelles ranures de temps on hi ha disponibilitat així bloquejant aquells moments on hi hagui interferències per evitar-les.
...
L'algorisme, en aquest cas, és el \textit{channel selection algorithm \#2} i és més efectiu a evitar interferències i esvaniments per propagació multi-camí.
...
La pila pròpia de Bluetooth Low Energy està dividida en dues parts, tal i com es pot observar a la figura \ref{ble_stack}, el controlador i el \textit{host}. Aquestes dues parts són independents i utilitzen el protocol \textit{Host Controller Interface} (HCI d'ara endavant) per comunicar-se entre elles.
Aquest protocol pot estar implementat amb qualsevol protocol de transport físic com USB o UART.
La idea darrera de separar la pila en dos serveix per fer compatible xips fets per diferents fabricants.
...
És per això, que els canals d'anunci es troben en el buits que deixen els canals WiFi més comuns (1, 6 i 11), tal i com es veu a la figura \ref{BLE_Channels}.
...
El BLE és un dels protocols que tenen la tassa de transmissió física \footnote{La tassa de transmissió física és aquella a la que transmeten la ràdio, no confondre amb la tassa de dades d'aplicació \textit{Throughput}} més alta, i de fins a 1 Mbps originalment i de 2 Mbps en BLE 5.
...
\subsection{Host}
El protocol BLE es va dissenyar amb la flexibilitat en ment, és per això que l'estructura de la pila del host permet no utilitzar certes capes que es consideren opcionals tal i com es pot veure a la figura \ref{host_stack}.
...
Aquesta MTU es pot definir (dins dels límits establerts) independentment per cada connexió així flexibilitzant l'ús que es dona al protocol en cada cas.
...
El \textit{Attribute Protocol} (ATT d'ara endavant) és el protocol d'aplicació més comú per a BLE i el \textit{Generic Attribute Profile} (GATT d'ara endavant) defineix com utilitzar el protocol per oferir serveis a capes superiors.
...
L'UUID (\textit{Universal Unique IDentifier}) identifica el tipus d'atribut, aquest numero pot ser de 16 bits si s'utilitza algun que ja està estandarditzat pel SIG o bé en tindrà 128 bits si està definit pel fabricant.
...
La definició de l'ATT és massa genèrica per si sola tal que seria comú que, per fer el mateix, es desenvolupessin múltiples definicions que fossin incompatibles entre sí.
...
En aquest exemple de la taula \ref{Attribute_Table} es pot observar que hi ha 2 serveis diferents, marcats en vermell, ja que hi ha 2 UUIDs 0x2800.
...
Tot i així es considera del tot improbable que es produeixin col·lisions degut a la longitud de 128 bits.
...
Aquests, per exmple, serveixen per poder subscriures a rebre notificacions cada cop que una característica canvia de valor.
...
\subsubsection{GAP}
\label{GAP}
La \textit{Generic Access Profile} és la capa que interactua amb la aplicació i per tant ofereix la Interfície de Programació d'Aplicacions (API en anglès) amb la funcionalitat que aporta BLE.
...
Amb aquests rols i la especificació del GAP existeix l'estandàrd que permet els dispositius descobrir-se, connectar-se  i emparellar-se entre d'altres.


\section{Anunciaments}
Quan els dispositius volen transmetre informació o volen connectar-se entre sí el primer que cal fer és anunciar-se.
...
L'ADV\_DIRECT\_IND serveix per anunciar-se a un dispositiu específic, per exemple, si un rellotge intel·ligent es vol connectar al telèfon al que està associat.
...
L'AUX\_ADV\_IND és el paquet que s'envia per un canal secundari després que s'hagui indicat pel tipus de paquet anterior.
...
\begin{figure}[h!]
	\begin{center}
		...
		\caption{Exemple d'anunci estès}
...
L'especificació de BLE defineix que pot estar entre 20ms i 10.24s amb salts de 0.625ms.
...
Tot i que, es pot definir el valor de l'interval d'anunci cal tenir en compte que segons l'especificació de BLE s'aplica un retard pseudo-aleatori que pot ser de fins a 10 ms i afecta als serveis que requereixen molt poca latència.
...
Això pot passar per exemple en entorns molt sorollosos o més sovint quant els dispositius surten del seu abast i el senyal no és detectable.
...
En la PDU (Protocol Data Unit) és on hi ha la informació pròpia del paquet i depenent de si és un paquet en un canal d'anunci o en un de dades té un format lleugerament diferent.
