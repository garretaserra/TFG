...
El desenvolupament de tecnologia per a connexions de curt abast que acabarien esdevenint el que avui es coneix com a Bluetooth va començar l'any 1989 per part de Ericsson.
...
Els membres del SIG després de negociar entre si van acordar incloure Wibree al estandard de bluetooth en la especificació 4.0 amb el nom de \textit{Bluetooth ultra low power technology} i publicitat com a Bluetooth Smart. El primer mòbil a incloure'l va ser el iPhone 4S al 2011.
...
\section{MANETS}
Bluetooth clàssic no permetia tenir més d'una connexió establerta amb dispositius i l'arquitectura feia que s'utilitzés bastanta energia per establir i mantenir aquesta connexió encara que es volguessin transmetre molt poques dades.
...
Amb l'arribada dels telèfons intel·ligents els auriculars inal·làmbrics es van fer populars i també es va establir la dominància de Bluetooth per a escoltar música sense cables.
...
Era necessari tenir xarxes sense fils, amb consum molt baix d'energia i que cobrissin una gran distància.

Amb aquests requeriments van sorgir les MANETS (Mobile Ad-Hoc Networks). A continuació s'explicaran les característiques de les tecnologies més importants amb BLE per veure on queda situat BLE.
...
Posteriorment, pel canal per on s'ha indicat s'envia el paquet que pot tenir fins a 255 octets de longitud total.
Amb aquest sistema s'aconsegueix descarregar els canals d'anunci passant informació als canals secundaris (0-36).
...
Si encara es requereix transmetre més dades el es pot fer utilitzant encadenament de paquets.
Al final del paquet que s'ha enviat pel canal secundari s'indica el canal per on s'enviara la continuació i això es pot fer múltiples vegades.
